...
2. Backend calls Billing `POST /v1/*` ([Node](/api/nodejs) / [Python](/api/python) / [Go](/api/go) / HTTP) and returns `checkoutUrl` (or payment link `url`) plus deep-link return URLs.

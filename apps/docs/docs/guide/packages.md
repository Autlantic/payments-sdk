...
Official **Python / Go / PHP / Java / .NET** server clients and **iOS / Android / Flutter / React Native** Checkout SDKs live under [`sdks/`](https://github.com/Autlantic/payments-sdk/tree/main/sdks). Published: npm (`@autlantic/payments-recurring`, `@autlantic/checkout`), PyPI (`autlantic-billing`), NuGet (`Autlantic.Billing`), pub.dev (`autlantic_checkout`), Go and iOS Git tags; PHP via [`billing-php`](https://github.com/Autlantic/billing-php) (Packagist pending); Java / Android via source until Maven Central. See [Languages and SDKs](/guide/languages) and [Mobile apps](/guide/mobile).
